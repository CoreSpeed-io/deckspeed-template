...
function lintMetadata(): void {
  ...
  if (!fs.existsSync(schemaPath)) {
    printError("metadata.schema.json not found", `Expected location: ${schemaPath}`);
    process.exit(1);
  }
  ...
  try {
    ...
    printError("Invalid JSON format in metadata.json", (error as Error).message);
    process.exit(1);
  }
  ...
  try {
    ...
    printError("Invalid JSON format in metadata.schema.json", (error as Error).message);
    process.exit(1);
  }
  ...
  try {
    ...
    slideIds.forEach(slideId => {
      ...
      if (!slideFiles.includes(expectedFilename)) {
        errors.push(`Slide with ID "${slideId}" does not have a corresponding file "${expectedFilename}"`);
      }
    });
    ...
    slideFiles.forEach((file: string) => {
      ...
      if (!slideIds.includes(slideId)) {
        errors.push(`TSX file "${file}" exists in slides directory but is not referenced in metadata.json`);
      }
    });
    ...
    printError("An unexpected error occurred while validating metadata.json", (error as Error).message);
    process.exit(1);
  }
}
